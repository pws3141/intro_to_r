...
Dataframes are ways to store different types of data in one place - similar to how SAS datasets contain different types of data (numeric, character, logical, dates *etc.*). Each column must be the same length.
...
str(df) # <1>
```
1. The structure function `str()` is very useful for looking at data.
...
glimpse(mpg)
```
As is the `str`ucture function, `str()`, which I frequently use to understand a dataset, or an output from a function.
...
Here we will look at loading data from excel, a csv file and from a SAS dataset.

### Reading an excel file

To read excel files, we will use the `read_excel()` function which is in the {realxl} package (part of the tidyverse). Although {realxl} is part of the tidyverse, it is not a core tidyverse package, and therefore needs to be loaded explicity, using `library(tidyverse)`. The `read_excel()` function supports both the legacy `.xls` format and the modern `.xlsx` format.

```{r}
library(readxl)
```

We are going to download an excel file and then read it into R. 
I have created an excel file (`.xlsx`) using the `mtcars` dataset. Download this file by clicking the button below:
...
mtcars %>%
  download_this(
    output_name = "example_data_set",
    output_extension = ".xlsx",
    button_label = "Download excel data",
    button_type = "warning",
    has_icon = TRUE,
    icon = "fa fa-save"
  )
...
Have a look at the `read_excel()` helpfile, and note that the `path` argument is the only one that doesn't have a default value. This means we need to assign something to it. The excel spreadsheet you just downloaded should be called `example_data_set.xlsx`, and we have two options to make sure R knows where it is:

1. Move the `example_data_set.xlsx` file to the folder that R is currently working in. This can be seen by calling `getwd()`.
2. Put the path of the `example_data_set.xlsx` file into the `path` argument of `read_excel()`.

For option one, to load this excel spreadsheet into R, we would move the file into our current working folder and then do the following:
```{r}
example_excel1 <- read_excel(path = "example_data_set.xlsx")
```

For option two, we can use the full path of the file.
```{r}
#| eval: false

example_excel2 <- read_excel(
  path = "C:/Users/smit0335/Downloads/example_data_set.xlsx" # <1>
  ) 
```
1. Your absolute path will be different to mine! We need to use forward slashes here, `/`, not backslashes.

You can also use relative paths, for example setting `path = "../data/example_data_set.xlsx"` would go up one folder, then into the "data" folder, and then read the excel spreadsheet from there.

Have a look at the data we've just loaded.
...
This is very similar to reading in an excel file, except we now use the `read_csv` function (this is from the {readr} package, which is included in the core tidyverse package^[That is, `library(tidyverse)` also loads {readr}.]). Download the below dataset:
...
If you open this in Notepad (right click, 'open with', 'Notepad'), you will see that the every element is seperated by a semicolon, `;`. Often in `.csv` files, the deliminator is either a comma, `,`, or a semicolon, `;`, so we need to make sure this is sepecified when loading in the dataset. You will also see that decimal points are given by commas, `,`.

Now, the description in the help file `?read_csv`, says 

> `read_csv2()` uses `;` for the field separator and `,` for the decimal point.

This is what we have, so lets use `read_csv2()` instead!

```{r}
example_csv <- read_csv2("example_csv_data.csv")
```
This message output is telling us the following:

- The dataset contains 32 rows and 11 columns.
- The delimited is `;`.
- `dbl` (double): all 11 columns are interpreted as numeric values of type double (floating-point numbers).

 We will discuss different types of variables later in this tutorial.
...
We can specify the column types when loading in the dataset. Types of data are discussed later on in this tutorial. The dataset in `example_csv` is the `mtcars` dataset, so by looking at the helpfile, `?mtcars`, I think the `cyl`, `gear` and `carb` variables should be factors and the `vs`, `am` variables should be logical.

To specify this when loading the data, we would use the `col_types` argument and the compact string representation as discussed in the help file, `?read_delim`.

```{r}
#| warning: false

read_csv2("example_csv_data.csv", col_types = "dfdddddllff")
```
:::

### Reading a SAS data file

To read in a SAS data file -- a file that ends in `.sas7bdat` -- we need to use the {haven} package [@wickham2023haven]. This is part of the tidyverse but needs to be loaded separately.
```{r}
library(haven)
```

The [package overview](https://haven.tidyverse.org/index.html) says:

> `read_sas()` reads `.sas7bdat` + `.sas7bcat` files and `read_xpt()` reads SAS transport files (versions 5 and 8). `write_xpt()` writes SAS transport files (versions 5 and 8).

Here, we're just going to load the `iris` dataset that is included as an example in the {haven} package. We use the `system.file()` to obtain the path where the dataset is saved.

```{r}
path <- system.file("examples", "iris.sas7bdat", package = "haven") # <1>
my_sas_dataset <- read_sas(path)
my_sas_dataset # <2>
```
1. You can see what this path is by typing `path` in the console.
2. Notice that the output has automatically set some columns to the `dbl` and the `Species` column to be `chr` (*i.e.* a string).

We can also create SAS transport files from R using the `write_xpt()` function. Don't use `write_sas()`, as it is depreciated, as described in the help file, `?write_sas()`.
...
# Linear and Logistic Regression {#sec-gettingStarted-lin-log-reg}

We will continue looking at the `penguins` dataset here. First, lets get some summary statistics using the `summary()` function.
...
To perform linear regression, we can use the `lm()` function. The two main arguments of `lm()` are:
...
Have a look at the helpfile, `?lm`, alongside the formula helpfile, `?formula`, to get a deeper understanding of how the function works. The `formula` syntax is also used in survival analysis in @sec-gettingStarted-surv, so is important to understand. 

### A simple linear model

Lets perform linear analysis on the penguin's body mass and flipper length. We have seen this graphically in @fig-ggplot-outcome but we want to look in more detail at the intercepts and slope coefficients. We are assuming the linear model given by,

$$
Y_{\text{body mass}} = \beta_0 + \beta_1 X_{\text{flipper length}} + \varepsilon,
$$
where $\beta_0$ is the intercept and $\beta_1$ the coefficient for flipper length such that for every 1mm increase in flipper length, we expect body mass to increase by $\beta_1$. We assume that the residual term is distributed by some zero-mean Gaussian, $\varepsilon \sim N(0, \sigma_\varepsilon)$.

We have `body_mass_g` the dependent variable and `flipper_length_mm` the independent variable, so our `formula` is `body_mass_g ~ flipper_length_mm`.
...
Note that with the `lm()` function, by looking at the help file, we can see that the `data = ` argument is not the first one. This is different to how standard tidyverse function work, *e.g.* have a look at `?mutate`.^[The `lm()` function was created long before the tidyverse so doesn't follow the same rules.] This makes it a little trickier to use the pipe `|>`, as by default this pipes the left-hand side into the first argument of the right-hand side.

But, it can still be done, using `_` to clarify where we want the data piped to.
...
A more detailed output can be obtained using the `summary()` function.^[`summary()` is a very general and very useful function that can be used on lots of different classes of data. For example, at the start of this chapter (@sec-gettingStarted-lin-log-reg) we applied the function to a tibble.]
...
- *Residuals*: a summary of the residuals, showing the minimum, maximum and quartiles.
- *Coefficients*: 
  - the point estimate and standard error for the intercept and slope.
  - the t-value and p-value of the intercept and slopes
- *Residual standard error*: the standard deviation of the residuals, *i.e.* the typical deviation of the observed `body_mass_g` values from the predicted values.
- *R-squared and adjusted R-squared*: how much variability is explained by the model.
- *F-statistic*: testing overall model fit.

This linear model is the same as the line shown in @fig-ggplot-outcome.

#### Evaluation model fit

We can perform residual analysis by looking at plots, as described on the . The `plot()` function with the `penguins_lm` object outputs four sepearate plots (which you can toggle through by pressing `<Return>`):

1. *Residuals vs. Fitted plot*: used to look for underlying patterns in the residuals.
2. *Normal Q–Q plot*: used to check whether the residuals are normally distributed.
3. *Scale-location plot*: test the linear regression assumption of equal variance (homoscedasticity).
4. *Residuals vs. Leverage plot*: used to find influential cases in the dataset.

These are discussed in more detail on the [RPubs website](https://rpubs.com/iabrady/residual-analysis).
...
### Some more linear model examples

Some more examples of linear models. 

```{r}
# modelling body mass by flipper length and sex.
penguins_lm2 <- lm(body_mass_g ~ flipper_length_mm + sex, data = penguins)
summary(penguins_lm2)

# modelling body mass by flipper length and the exponential log of bill_depth_mm
summary(lm(body_mass_g ~ flipper_length_mm + log(bill_depth_mm), data = penguins))
...
We can consider interactions, using `*`. This automatically includes the main effects in the model.[^note3]

[^note3]: You can also use `:` for interactions, but here you also have to specify the terms individually, e.g. 
    ```r
    lm(body_mass_g ~ flipper_length_mm + bill_depth_mm + flipper_length_mm : bill_depth_mm, 
        data = penguins)
    ```
    
```{r}
summary(lm(body_mass_g ~ flipper_length_mm * bill_depth_mm, data = penguins))
```
From this output, the interaction between flipper length and bill depth can be interpreted as follows: For every 1mm increase in both flipper length and bill depth, the predicted effect on body mass decreases by $8.596$ grams. 
The very small p-value ($4.78 \times 10^{−10}$) indicates strong evidence that the relationship between flipper length and body mass depends on bill depth.

## Logistic regression

The syntax for (binary) logistic regression is very similar to that for linear regression, except that the `g`eneralised `l`inear `m`odel function, `glm()` is used instead of `lm()`.

In this section we'll be assuming that the dependent variable is `sex` -- a binary variable. Let start with just considering `sex` against `bill_length_mm`.

```{r}
ggplot(penguins, aes(x = bill_length_mm, y = sex)) +
  geom_point() +
  labs(
    title = "Males have larger bill lengths in general",
    subtitle = "A graph showing bill length, split by sex",
    x = "Bill length (mm)",
    y = "Sex"
  ) +
  theme_minimal()
...
I'm getting bored of the warnings about missing values, so lets remove all rows that contain an `NA` from our dataset.[^note5]

[^note5]: You should probably take care doing this if you are trying to do proper statistical analysis. There is a package called [`visdat`](https://cran.r-project.org/web/packages/visdat/vignettes/using_visdat.html) that can help you visualise data. In this case, it can create a plot showing where the missing data is and the class of each variable. Notice that here our `year` variable is of class integer, not date. If we had a full date (e.g. DD/MM/YY) we could change this to a Date class and use the [{lubridate}](https://lubridate.tidyverse.org/) package to perform operations.
    ```{r}
    #| lightbox: true
    library(visdat)
    vis_dat(penguins)
...
Here we create a logistic regression model to predict the gender of the penguins.

```{r}
penguins_glm <- glm(sex ~ bill_length_mm, 
                    data = penguins_narm, family = binomial(link = "logit")) # <1>
summary(penguins_glm)
```
1. A logistic regression model is a specific generalised linear model with the response variable modeled using a binomial distribution and a logit link function.

The model suggests that `bill_length_mm` is a significant predictor of `sex`. Specifically:

- For every 1-mm increase in bill_length_mm, the log odds of being in the other category ("Male") increase by $0.138$.
- The coefficient is statistically significant, and therefore out model suggests that `bill_length_mm` is associated with `sex`.
- The odds ratio for `bill_length_mm` is $e^{0.1377} \approx `r round(exp(penguins_glm$coefficients[2]), 3)`$.[^note6] This means that for every 1mm increase in bill length, the odds of being male are approximately $`r (round(exp(penguins_glm$coefficients[2]), 3) - 1) * 100` \%$ higher.

[^note6]: The `str()` function comes in very handy when extracting specific bits of data from a dataset. Try typing `str(penguins_glm)`. This is a lot of information, but it allows us to see that the coefficients are stored in a vector called `coefficient`. This can be extracted using `penguins_glm$coefficients`. Therefore, `exp(penguins_glm$coefficients[2])` gives the odds ratio for `bill_length_mm`.

::: {.callout-note collapse="true"}
# Changing the reference level

If we wanted `male` to be the reference level instead of `female`, we can reorder the `sex` factor levels using `fct_relevel()`.

```{r}
# look at the levels
levels(penguins_narm$sex) # <1>
```
1. The first factor in the output of `levels()` is used as the reference factor in `lm()` and `glm()`.
```{r}
# relevel 'sex' to make "male" the reference class
penguins_narm_relevel <- penguins_narm |>
  mutate(
    sex = fct_relevel(sex, "male")
  )
```
```{r}
# check the level order has changed
levels(penguins_narm_relevel$sex)
```
```{r}
penguins_glm <- glm(sex ~ bill_length_mm, 
                    data = penguins_narm, family = binomial(link = "logit")) 
summary(penguins_glm)
```
...
# Performing survival analysis {#sec-gettingStarted-surv}
...
*A question*: what would the output look like if we didn't record the `status` variable to 0 / 1? Would it make a different?
:::
